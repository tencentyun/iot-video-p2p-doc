...
先通过获取设备属性数据API（ https://cloud.tencent.com/document/product/1131/53100 ) 获取"sys_p2p_info" 字段，"sys_p2p_info"字段样式如：XP2Pxxxxx%x.y.z，其中前4个字节为固定为”XP2P“，第5个字节到%(即：xxxx位置)为此版本的设备端”peername“信息，该字段填入到小程序中的”peername“中。（!注：调用P2P插件接口时直接将云API获取的字段传给P2P插件localpeername即可）
