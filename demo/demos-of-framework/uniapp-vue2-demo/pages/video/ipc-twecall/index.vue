	...
	<view class="twecall-wrap">
		...
		<van-cell-group title="4、参数信息">
			...
			<van-field
				label="openId"
				right-icon="search"
				:value="voipOptions.openId"
				@clickIcon="handleCopy('openId')"
			/>
		</van-cell-group>
		<view class="btn-group">
			<van-button
				type="primary"
				@click="requestDeviceAuth"
				>查询授权状态</van-button
			>
			<van-button
				type="primary"
				@click="navigateEndPage"
				>去通话结束页</van-button
			>
		</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			handleCopy(type) {
				const copyData =
					type === "snTicket" ? this.voipOptions.snTicket : this.voipOptions.openId
